...
class FAL(ABC):
    ...
    def exists(self, path: str) -> int:
        """
        Check resource existence and return its status or size.

        Returns:
            int: -1 if missing, 0 if directory, or file size in bytes.
        """
...
class FAL_RA(FAL):
    ...
    def create(self, path: str, content):
        ...
        with open(path, "w") as f:
            f.write(content)
        # Later requests may hit the same file from different services/users.
        os.chmod(path, 0o777)
